...
//	exit();
	...
	{		
		...
		if ($Fecha_Inicio_Etapa=='')
		{
				$ban_noetapa=1;
				$dia_del_mes=1;
				$Nombre=$contajob.".- ".$Nombre."/Super:".substr($Nsuper,0,7);
				//." ".$Pro_ID;
				$contajob++;			
		...
		if ($Note<>"")
			$Nombre=substr($Nombre,0,11)."/".$Note;
	...
	echo " Man Power Projection by Month and by day ==:";
	...
	while ( $i<$ultimo_dia )
	{
		...
		$totalmes=$totalmes+$total_dia[$i];
		$totalactmes=$totalactmes+$totalactdia[$i];
	...
	$pdf->SetY($aux+25);
	$pdf->SetX(50);
	$pdf->Multicell(65,5,"//Averange*1.10=".round($avera*1.1),0,C,true)."/Averange Act.:".round($totalactmes/$dialab);
		
	echo '</tfoot>';		
	echo "</table>";
	$avera=$totalmes/$dialab;
	echo "Work Days".$dialab." Total Man days projected in the month:".round($totalmes)." / Total Man days worked: ".round($totalactmes)."<br>";
	echo " Averange Projected:".round($avera)."<br>";
	echo "//Averange*1.10=".round($avera*1.1)."/Averange Actual:".round($totalactmes/$dialab)."<br>";
