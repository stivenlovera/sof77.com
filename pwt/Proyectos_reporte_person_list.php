<?php
	...
	function encabezado(&$pdf,$f1,$f2)
	{
		...
		$pdf->Multicell(0,5,"Daily Repor by PWT Superintendent: from ".$f1."   to: ".$f2,0,C,false);
...
/*		$aux=$pdf->GetY();
	...
	$sql = "SELECT p.*,e.Nombre_Estatus, em.Nombre as Company, ";	
	...
	$sql = $sql . " INNER JOIN estatus e on e.Estatus_ID=p.Estatus_ID ";	
...
//echo $sql."<br>";
    ...
    while($row00=mysqli_fetch_array($result_00))
	{
		...
		$pdf->SetX(10);
		$numjob++;	
		$pdf->Multicell(70,5,$numjob." .-Job ".$Codigo." ".$Nombre,0,L,'R'); 	
		$pdf->SetY($aux);				
		$pdf->SetX(85);	
		...
		$pdf->SetY($aux);				
		$pdf->SetX(175);			
		$pdf->Multicell(35,5,"Status: ".$Estatus,0,R,false);
		//$pdf->SetX(210);	
		...
		$pdf->Multicell(50,5,"PWT: ".$Texta,0,L,false); 	
		$pdf->SetY($aux);				
		$pdf->SetX(60);	
		$pdf->Multicell(50,5,"Foreman PWT: ".$Foreman,0,L,false); 
 ...
 if ($Super_ID=="")
	...
	while (($row33 = mysqli_fetch_array($result33) ))							
		{				
			...
			$pdf->SetY($aux);
			$pdf->SetX(115);	
			$pdf->Multicell(100,5,"GC.: ".$Company." Contact: ".$Super,0,L,false);
		}

			/*$aux=$pdf->GetY();
			$pdf->SetY($aux);
			$pdf->SetX(20);	
			$pdf->Multicell(120,5,"Notes: ".$Nota,0,L,false);*/



/// get hours used by cost code 
		$sql1_1 = "select sum(Horas_Contract) as HrsUse,a.Nombre as ANombre, t.Nombre as TNombre,t.Last_Per_Recorded as LPRec,t.Last_Date_Per_Recorded as LDPer,t.Note from registro_diario_actividad rda inner join registro_diario rd on rda.Reg_ID=rd.Reg_ID inner join task t on t.Task_ID=rda.Task_ID Inner Join area_control a on a.Area_ID=t.Area_ID  WHERE rd.Pro_ID=".$Pro_ID." and rd.Fecha between "."'".$af1."' AND '".$af2."' group by rda.Task_ID  order by a.Nombre,t.Task_ID" ;
				//echo $sql1_1;
				//exit ();
		...
		$pdf->SetY($aux);  
		$pdf->SetX(5);	
		$pdf->Multicell(120,5,"Worked at:");
		$result11=$bd->ejecutar($sql1_1);	
		while (($row11 = mysqli_fetch_array($result11) ))							
			{				
				$ANombre= $row11["ANombre"];
				$TNombre= $row11["TNombre"];
				$HrsUse=$row11["HrsUse"];
				$LDPer=$row11["LDPer"];
				$LPRec=$row11["LPRec"];
				$Note=$row11["Note"];
				...
				$pdf->SetY($aux);  
				$pdf->SetX(10);	
				$pdf->Multicell(30,4,"Area: ".$ANombre);
				$pdf->SetY($aux); 
				$pdf->SetX(50);	
				$pdf->Multicell(50,4,"Task:".ltrim($TNombre));
				$pdf->SetY($aux); 
				$pdf->SetX(120);	
				$pdf->Multicell(15,4,$HrsUse."Hrs.",0,L,false);
				$pdf->SetY($aux); 
				$pdf->SetX(137);	
				$pdf->Multicell(95,4,"  Percent Completed: ".$LPRec."%  On:".$LDPer);
				$aux=$pdf->GetY();
				$pdf->SetY($aux);  
				$pdf->SetX(120);	
				$pdf->Multicell(100,4,"Report: ".$Note);
			}
