		...
		switch ($j) 
		{
			...
			case 1:
				...
				//------
							...
							$auxxd=$auxxd+7;
							$pdf->SetX($auxxd);
							$pdf->Multicell(7,7,"Tu",1,C,false);
			...
			case 3:
				...
				//------
							...
							$auxxd=$auxxd+7;
							$pdf->SetX($auxxd);
							$pdf->Multicell(7,7,"Th",1,C,false);
...
//	$pdf->line(10,$pdf->GetY()-5,280,$pdf->GetY()-5);
	...
	while ($dia_actual <= $ultimo_dia)
	{
		...
		//------
							...
							$auxxd=$auxxd+7;
							$pdf->SetX($auxxd);
							$pdf->Multicell(7,5,$dia_actual,0,C,false);
...
imageline($im, 0, 0, 300, 500, IMG_COLOR_STYLED);
...
////------------------
	...
	$consulta = "SELECT p.Pro_ID,p.Nombre, e.Etapas_ID, e. Nombre as Nombre_Etapa,  e.Fecha_Inicio AS Fecha_Inicio_Etapa, e.Fecha_Fin AS Fecha_Fin_Etapa,e.Note as Note, 
					...
					WHERE 
							...
							)  AND ( Estatus_ID<5) OR (EXISTS (SELECT a.Pro_ID,a.Fecha from actividades a where a.Pro_ID=p.Pro_ID and a.Fecha BETWEEN '".$ano."-".$mes."-01 00:00:00' AND '".$ano."-".$mes."-31 23:59:59') ) 
	...
	{		
		...
		//-----
				...
				$pdf->SetY($aux);
				$pdf->SetX(5);
				$pdf->Multicell(150,3,$Nombre,1,L,false);
				$aux=$aux+10;
				$auxx=0;
				if ($aux>70)
					{
		...
		/*		//-----
				...
				$pdf->SetY($aux);
				$pdf->SetX(5);
				$pdf->Multicell(150,3,$Nombreact,1,L,false);
				$aux=$aux+10;
				$auxx=0;
				if ($aux>70)
					{
				...
				//-----*/
							...
							$auxx=$auxx+7;
							$pdf->SetX($auxx);
							$pdf->Multicell(7,7,$Empleadosx,0,L,false);
		...
		if ($Pro_ID!=$Pro_ID_ant)
		{
			...
			{		
				...
				while ($dia_del_mes<=$ultimo_dia) 
				{
					...
					//[[[[[[[[[[[[[[[[[[[
							...
							$auxx=$auxx+7;
							$pdf->SetX($auxx);
							$pdf->Multicell(7,7,$Empleadosx,1,L,false);
			...
			/*//-----
				...
				$pdf->SetY($aux);
				$pdf->SetX(5);
				$pdf->Multicell(150,10,$Nombre,0,L,false);
				$auxx=0;
				$aux=$aux+10;
				if ($aux>40)
					{
			...
			if ($dias_despues>=0)
			{
				...
				if (Es_Dia_Habil($Fecha_Nuevo_Dia, $bd))
				{
					...
					//-----------
							...
							$auxx=$auxx+7;
							$pdf->SetX($auxx);
							$pdf->Multicell(7,7,$Empleados,0,L,false);
...
//					echo "<td align='center' $Estilo ><a href='#' onclick='Actividad_Nueva(\"$Fecha_Nuevo_Dia\", $Pro_ID);'>$Empleados_Diarios</a></td>";	
	...
	echo " Man Power Chart: Actual and Projection Montly ==:";
	//---------
	
	$pdf->SetY($aux);
	$pdf->SetX(100);
	$pdf->Multicell(0,5,"Man Power Projection by Month and by day ==:",0,C,false);
