<?php
...

	{
	  ...
	  $pdf->Multicell(0,5,"Daily Report",0,C,false);
		...
		{$pdf->Multicell(0,5," ".$f1,0,C,false);}
...
//				  $fecha=date_format($dato,'y/m/d');
				  ...
				  $pdf->Multicell(70,5,$status."/".$row['codigo'].",".$row['nombre'].", Add:".$row['calle'].",".$row['ciudad'].",".$row['estado'].",".$row['zip_code']." Contac:".$vcontacto,0,L,true);			  
...

			{
				...
				$pdf->Multicell(45,5,"-".$row4['NHoras'],0,L,false);	
				...
				$aux7=$pdf->GetY();		
				  ...
				  //************************************* INICIO Horas de Trabajo *************************************				  
					...
					$pdf->Multicell(50,5,$Detalleh,0,L,true);
					...
					$pdf->Multicell(180,5,$Detalle,0,L,false);
					...
					$consulta = $consulta . " CONCAT(em5.Nombre, ' ',  em5.Apellido_Paterno, ' ',  em5.Apellido_Materno) as Coordinador_Obra, em5.Telefono as TelefonoC,  em5.Celular as  CelularC  FROM proyectos p ";
...

						{
						  ...
						  Else
							...
							$Detalle = "Hours Worked in Contract:".$HContract."h";
...
/*
					...
					{		
						...
						$Detalle = Substr($Denominacion,0,10)."...:   Estimated ".$Cantidad.$Unidad_Medida."    Ordered ".$Total_Cantidad_Ordenada.$Unidad_Medida;
						...
						$Detalle = $Detalle ."    %Used ".$Por_Usada."%       %Ordered ".$Por_Comprar;   
				  ...
				  //************************************* INICIO Material Request *************************************				  
					...
					$pdf->Multicell(30,5,"Material Requested:",0,L,true);	
	...
	<embed src="dato.pdf#toolbar=1&navpanes=0&scrollbar=1" width="1190" height="800"></embed>
