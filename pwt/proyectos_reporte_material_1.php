<?php
	...
	function encabezado(&$pdf,$f1,$f2)
	{
	  ...
	  $pdf->Multicell(0,5,"Detail of materials ordered and used",0,C,false);
  	  $pdf->Multicell(0,5,"from: ".$f1."   to: ".$f2,0,C,false);
  	  $pdf->Multicell(0,5,"",0,L,false);
	  // titulo del detall	  
  	  $aux=$pdf->GetY();
	  $pdf->SetX(20);
	  $pdf->Multicell(20,5,"Date",0,L,false);
	  $pdf->SetY($aux);
	  $pdf->SetX(40);
	  $pdf->Multicell(80,5,"Description",0,L,false);
	  $pdf->SetY($aux);
	  $pdf->SetX(120);
	  $pdf->Multicell(15,5,"U.Med.",0,L,false);
	  $pdf->SetY($aux);
	  $pdf->SetX(142);
	  ...
	  $pdf->SetY($aux);
  	  $pdf->SetX(235);
	  $pdf->Multicell(25,5,"Received - Used",0,C,false);
	  $aux7=$pdf->GetY();
	...
	function subtotal(&$pdf,&$parcial_ordenado,&$parcial_recibido,&$parcial_usado,&$parcial_balance,&$parcial_balance_1,&$parcial_balance_2)
	{
	  $aux=$pdf->GetY();
  	  $pdf->line(120,$pdf->GetY(),265,$pdf->GetY());
	  $pdf->SetX(120);
	  ...
	  $pdf->SetY($aux);
	  $pdf->SetX(180);					  
	  $pdf->Multicell(25,5,$parcial_usado,0,R,false);
	  $pdf->SetY($aux);
	  $pdf->SetX(205);					  
	  $pdf->Multicell(25,5,$parcial_balance_1,0,R,false);	
	  $pdf->SetY($aux);
	  $pdf->SetX(230);					  
	  $pdf->Multicell(25,5,$parcial_balance_2,0,R,false);								
   	  //$pdf->line(120,$pdf->GetY(),260,$pdf->GetY());
   	  $pdf->Multicell(0,5,"",0,L,false);
	  $parcial_ordenado=0;
	  $parcial_recibido=0;
	  $parcial_usado=0;
...
//	  $af1=date_format($dato,'y/d/m');
    ...
    while($row0=mysqli_fetch_array($result_0))
	{
		...
		$pdf->Multicell(150,5,"Job: ".$Codigo." ".$Nombre,0,L,false); 
			...
			$sql = "select pedidos.fecha,materiales.denominacion,materiales.unidad_medida,pedidos_material.Aux1,pedidos_material.cantidad,pedidos_material.Mat_ID,pedidos_material.cantidad_recibida,pedidos_material.cantidad_usada from	proyectos INNER JOIN (pedidos INNER JOIN (pedidos_material INNER JOIN materiales on pedidos_material.mat_id=materiales.mat_id)	on pedidos.ped_id=pedidos_material.ped_id) on proyectos.pro_id=pedidos.pro_id WHERE proyectos.Pro_ID=$Pro_ID AND pedidos.fecha between '$af1' AND '$af2'";
			
			if ($Nombre_Material!="")	
				$sql = $sql . " AND materiales.denominacion like '%$Nombre_Material%' ";
		
			$sql = $sql . " ORDER BY pedidos_material.Mat_ID desc, materiales.denominacion,materiales.unidad_medida,pedidos.fecha ";
				...
				while($row=mysqli_fetch_array($result))
					{
						if(($vdenominacion!=$row["denominacion"] || $vunidad_medida!=$row["unidad_medida"]) && $bandera==0)
							{				
							 subtotal($pdf,$parcial_ordenado,$parcial_recibido,$parcial_usado,$parcial_balance,$parcial_balance_1,$parcial_balance_2);					 
							 $vdenominacion=$row["denominacion"]."/".$row["Aux1"];
						...
						if($bandera_2==1)
							...
							$aux=$pdf->GetX();
							$pdf->SetX(40);
							$pdf->Multicell(80,5,$row[denominacion],0,L,false);
							$bandera_2=0;							  
							}
						   //********************************************************************
						   // DETALLE
						  ...
						  $pdf->SetY($aux);
						  $pdf->SetX(40);
						  $pdf->Multicell(80,5,$row['denominacion']."/".$row[Aux1],0,L,false);
						  $aux33=$pdf->GetY();
						  $pdf->SetY($aux);					  
						  $pdf->SetX(120);
						  $pdf->Multicell(15,5,$row['unidad_medida'],0,L,false);
						  $pdf->SetY($aux);
						  $pdf->SetX(135);
						  $pdf->Multicell(25,5,$row['cantidad'],0,R,false);
						  $pdf->SetY($aux);
						  $pdf->SetX(160);					  
						  $pdf->Multicell(25,5,$row['cantidad_recibida'],0,R,false);
						  $pdf->SetY($aux);
						  $pdf->SetX(185);
						  ...
						  $total_1=$row['cantidad']-$row['cantidad_recibida'];
						  $total_2=$row['cantidad_recibida']-$row['cantidad_usada'];
						  	
						  $pdf->Multicell(25,5,$total_1,0,R,false);
						  $pdf->SetY($aux);
						  $pdf->SetX(235);
					  ...
					  $parcial_balance_1=$parcial_balance_1+$total_1;
					  $parcial_balance_2=$parcial_balance_2+$total_2;
					  ...
					  $total_balance_1=$total_balance_1+$total_1;
					  $total_balance_2=$total_balance_2+$total_2;
				  ...
				  $pdf->SetY($aux);
				  $pdf->SetX(205);					  		  
				  $pdf->Multicell(25,5,$total_balance_1,0,R,false);								
				  $pdf->SetY($aux);
				  $pdf->SetX(232);					  		  
				  $pdf->Multicell(25,5,$total_balance_2,0,R,false);
				  $pdf->SetY($pdf->GetY()+5);
			}	
	}
	$pdf->Output("dato.pdf");
