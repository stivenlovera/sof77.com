<?php
	...
	{	
		$eligio="No";
?>		
		<select name="Floor_ID_<?php echo $Fila; ?>" size="1"  class="cuadro" id="Floor_ID_<?php echo $Fila; ?>" onchange="foreman_registro_actividad_area(this.value,<?php echo $Fila; ?>,<?php echo $RDA_ID; ?>);">  
			<option value="-1">Select one</option>  		   
			<?php				
			$sql = "select Floor_ID, Nombre FROM floor WHERE Edificio_ID=".$Edificio_ID." and Nombre not like '%Material%' and Nombre not like '%Submittal%' and Nombre not like '%Closeout%' and Nombre not like '%SubCont%'  and Nombre not like '%do not use%' order by Nombre";														
			$result=$bd->ejecutar($sql); 		
	...
	<?php
		...
		{
	?>		
			<img src="images/spacer.gif" onload="foreman_registro_actividad_area(<?php echo $Floor_ID;?>,<?php echo $Fila; ?>,<?php echo $RDA_ID; ?>);" />
	<?php
		}
	}
	else
	{					   
		$sql = "select Floor_ID, Nombre FROM floor WHERE Edificio_ID=".$Edificio_ID." order by Nombre";															
		$result=$bd->ejecutar($sql); 		
		while (($row = mysqli_fetch_array($result) ))	
		{	
?>
			...
			<img src="images/spacer.gif" onload="foreman_registro_actividad_area(<?php echo $row["Floor_ID"];?>,<?php echo $Fila; ?>,<?php echo $RDA_ID; ?>);" />	 
