	...
	for(var no=0;no<divs.length;no++){
		...
		if(divs[no].className=='dhtmlgoodies_window'){	
			...
			for(var no2=0; no2 < subImages.length; no2++){
				...
				if(subImages[no2].className=='closeButtonDhtml'){
					subImages[no2].id = 'closeImage' + divCounter;
					subImages[no2].onclick = hideWindow;	
				}	
				if(subImages[no2].className=='minimizeButtonDhtml'){
...
function createNewWindow(width,height,left,top)
{
	...
	img.className='minimizeButtonDhtml';
	buttonDiv.appendChild(img);	
	
	
	var img = document.createElement('IMG');
	img.src = '../images/close.gif';
	img.className='closeButtonDhtml';
	buttonDiv.appendChild(img);	
	...
	windowPositionArray[windowSizeArray.length] = [left,top];
