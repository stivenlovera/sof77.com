...
$result=$bd->ejecutar("SELECT COUNT(*) AS count FROM invheader a, clients b WHERE a.client_id=b.client_id"; 	
...
$SQL = "SELECT p.*, e.Nombre as Company FROM proyectos p INNER JOIN empresas e ON p.Emp_ID=e.Emp_ID ORDER BY $sidx $sord LIMIT $start , $limit";
$result = $bd->ejecutar( $SQL ) or die("Couldn t execute query.".mysql_error());
...
while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
    $responce->rows[$i]['id']=$row[Pro_ID];
    $responce->rows[$i]['cell']=array($row[Pro_ID],$row[Codigo],$row[Nombre],$row[Fecha_Inicio],$row[Fecha_Fin],$row[Horas],$row[Estado]);
    $i++;
}
