...
function Update_Info_Percentage(id, Pro_ID, Task_ID, datesql, value, B_Las_Per, B_Las_Date, usr) 

	{
		...
		//eval(" $('#Note-"+Pro_ID+"-"+Empleado_ID+"-"+Actividad_ID+"').html('"+Note+"');");
		//window.alert(Note);
...
//	echo "<script type='text/javascript'>window.alert('llego  Job_Percentage_RECORD --!!--');<F/script>";
...
if ($datesqlx!=$datesql || $usrx!=$usr)
		$strSQL = "UPDATE task SET B_Last_Percentage=Last_Per_recorded,B_Last_Date=Last_Date_Per_Recorded,BUsr=Usr,".$campo."='".$value."',Last_Date_Per_Recorded='".$datesql."',Usr='".$usr."'  WHERE Task_ID=".$Task_ID;					
	else 
