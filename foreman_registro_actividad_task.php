...
?>		 
		<select name="Task_ID_<?php echo $Fila; ?>" size="1"  class="cuadro" id="Task_ID_<?php echo $Fila; ?>" >  
			...
			<?php				
			$sql = "select Task_ID, Nombre FROM task WHERE Area_ID=".$Area_ID." and Nombre not like '%Material%' and Nombre not like '%Submittal%' and Nombre not like '%Closeout%' and Nombre not like '%SubCont%' and Nombre not like '%do not use%' order by NumAct";														
			$result=$bd->ejecutar($sql); 		
...
<?php
	...
	{					   
		$sql = "select Task_ID, Nombre FROM task WHERE Area_ID=".$Area_ID." order by Nombre";																
		$result=$bd->ejecutar($sql); 		
		while (($row = mysqli_fetch_array($result) ))	
		{	
?>
