...
?>		 
		<select name="Task_ID_<?php echo $Fila; ?>" size="1"  class="cuadro" id="Task_ID_<?php echo $Fila; ?>" >  
			...
			<?php				
			$sql = "select Task_ID, RTRIM(Nombre) AS Nombre FROM task WHERE Area_ID=".$Area_ID." and Nombre not like '%Material%' and Nombre not like '%Submittal%' and Nombre not like '%Closeout%' and Nombre not like '%SubCont%'  and Nombre not like '%do not use%' order by Nombre";														
			$result=$bd->ejecutar($sql); 		
			...
			{	
				if ($Task_ID==$row["Task_ID"])
					...
					$estado="";
			?>
				<option value="<?php echo $row["Task_ID"];?>" <?php echo $estado;?> ><?php echo $row["Nombre"];?></option>  
			<?php
			}
			mysqli_free_result($result);	
			?>
...
<?php
	...
	{					   
		$sql = "select Task_ID, RTRIM(Nombre) AS Nombre FROM task WHERE Area_ID=".$Area_ID." order by Nombre";																
		$result=$bd->ejecutar($sql); 		
		while (($row = mysqli_fetch_array($result) ))	
		{	
?>
			<input type="text" value="<?php echo $row["Nombre"];?>" size="15" maxlength="25" />
