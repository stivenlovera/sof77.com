...
//		$pdf->Multicell(22,5,"Employee        #",0,L,false);
		...
		$pdf->SetY($aux);
		$pdf->SetX(154);
		$pdf->Multicell(55,5,"Area-CostCode-Descrip. ",0,L,false);
		...
		$pdf->SetY($aux);
		$pdf->SetX(262);
		$pdf->Multicell(25,5,"Gas/Week",0,C,false);
		
		$pdf->SetY($aux);
		$pdf->SetX(290);
		$pdf->Multicell(25,5,"Other Benefits",0,C,false);
...
/*
	...
	function pie(&$pdf,$Total_Horas_Contract, $Total_Horas_TM,$Total_Gas,$BenefitA,$BenefitB)
	{
		...
		$aux=$pdf->GetY();
		$pdf->SetX(170);
		$pdf->Multicell(20,5,"TOTAL: ",0,R,false);	
		...
		$pdf->SetY($aux);
		$pdf->SetX(285);
		$pdf->Multicell(50,5,$BenefitA."//".$BenefitB,0,R,false);
...
*/
	...
	$filename=$filename.$vmes."-".$vdia."-".$vano;
	...
	encabezado2($pdf,$af1,$af2);
		...
		///////////
		$sql="SELECT ap.*,rda.*,a.*,rd.*,p.*,t.*,ap.Empleado_ID AS EmpleadoIDx,p.Extra_Mon1,p.Benefit1,p.Benefit2,p.Nombre AS Nombre_Empleado,p.Numero AS NumEmp, pr.Pro_ID,pr.Codigo, pr.Nombre AS Proyecto, rd.Hora_Ingreso,rd.Hora_Salida,t.NumAct AS CosCod,t.Tas_IDT AS TasIDT,ac.Fecha, rda.Horas_Contract AS HorasCont,rda.Horas_TM as HorasTM,";
		if ($filtro=="Resume")
		...
		$sql=$sql." WHERE 1=1 AND ac.Fecha BETWEEN '".$af1."' AND '".$af2."' ";
	...
	<?php
		...
		pie($pdf,$Total_Horas_Contract, $Total_Horas_TM,$Total_Gas,$BenefitA,$BenefitB);
