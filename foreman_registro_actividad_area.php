<?php
	...
	{	
		$eligio="No";
?>		
		<select name="Area_ID_<?php echo $Fila; ?>" size="1"  class="cuadro" id="Area_ID_<?php echo $Fila; ?>" onchange="foreman_registro_actividad_task(this.value,<?php echo $Fila ?>,<?php echo $RDA_ID; ?>);">  
		<option value="-1">Select one</option>  	   
		<?php				
		$sql = "select Area_ID, Nombre FROM area_control WHERE Floor_ID=".$Floor_ID." and Nombre not like '%Material%' and Nombre not like '%Submittal%' and Nombre not like '%Closeout%' and Nombre not like '%SubCont%' and Nombre not like '%do not use%' order by Nombre";														
		$result=$bd->ejecutar($sql); 		
  ...
  <?php
		...
		{
	?>		
			 <img src="images/spacer.gif" onload="foreman_registro_actividad_task(<?php echo $Area_ID;?>,<?php echo $Fila; ?>,<?php echo $RDA_ID; ?>);" />
	<?php
		} 
	}
	else
	{					   
		$sql = "select Area_ID, Nombre FROM area_control WHERE Floor_ID=".$Floor_ID." order by Nombre";																	
		$result=$bd->ejecutar($sql); 		
		while (($row = mysqli_fetch_array($result) ))	
		{	
?>
			...
			<img src="images/spacer.gif" onload="foreman_registro_actividad_task(<?php echo $row["Area_ID"];?>,<?php echo $Fila ?>,<?php echo $RDA_ID; ?>);" />			
<?php
		}
		mysqli_free_result($result);	
